#!/bin/bash
# Self-healing agent bridge check for the OSF devcontainer.
#
...
echo "osf: agent bridge sockets missing — restarting..."
...
while [ "$i" -lt 10 ]; do
    if [ -S /tmp/ssh-agent.sock ] && [ -S /tmp/S.gpg-agent ] && [ -S /tmp/S.scdaemon ]; then
        echo "osf: agent bridge restarted OK (log: /tmp/agent-bridge.log)"
        return 0 2>/dev/null || exit 0
    fi
    sleep 0.5
    i=$((i + 1))
...
echo "osf: WARNING agent bridge failed to start — SSH/GPG forwarding is DOWN."
echo "osf:   check the host relay: sh .devcontainer/scripts/install-agent-relay.sh status"
echo "osf:   container log:        /tmp/agent-bridge.log"
