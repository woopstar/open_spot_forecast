...
export MYPY_CACHE_DIR="${MYPY_CACHE_DIR:-/tmp/osf-cache/mypy}/${_worktree_id}"
export RUFF_CACHE_DIR="${RUFF_CACHE_DIR:-/tmp/osf-cache/ruff}/${_worktree_id}"
...
case "${1:-}" in
    ...
    quality)
        ...
        run python -m vulture custom_components/open_spot_forecast tests vulture_whitelist.py --min-confidence 80
        echo ""
        echo "[info] Supplementary vulture pass (custom_components/open_spot_forecast only, tests"
        ...
        run python -m vulture custom_components/open_spot_forecast vulture_whitelist.py --min-confidence 0 || true
        ;;
    format-check)
